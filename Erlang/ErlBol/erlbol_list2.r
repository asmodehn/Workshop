REBOL [Title: "Erlbol List Demo 2"
Author: "Doug Edmunds"
...
erl_functions: ["add_ints"  "add_numbers"  "mult_numbers"  "combine_strings" 
"make_list"  "make_tuple"  "make_mess"  "not_a_function"  ]
current-function: first erl_functions

process_it:
            [
            ;Must 'reduce' the block of fields
            prin "current-function in button " print current-function
            myfields:  reduce [current-function erl_field1/text erl_field2/text]
...
if error? try [  
  ...
  view layout [
        h2   "Erlbol List Demo 2 v 2008.05.25.01" 
        h3   "http://erlbol.douglasedmunds.com"
        text "Just click the function from list"
        tl: text-list data erl_functions [
             current-function: to-string value
             do process_it
             ]
        ;erl_function: field "add_ints"
        erl_field1: field  "4"
        erl_field2: field  "15"
        result1: text 200x80 wrap  "result1 area" 
        button "Quit" [quit]
        ] 
